...
export default (props) => {
  ...
  const onDelete = async (row) => {
    confirm({
      title: "Anda yakin akan menghapus supplier ini?",
      content: "Aksi tidak dapat dikembalikan",
      onOk() {
        return new Promise(async (resolve, reject) => {
          await api.delete(`suppliers?supplier_id=${row.id}`);
          resolve();
          refresh();
        });
      },
  ...
  return (
    <Content title="Supplier" backToDashboard>
      <Row gutter={[24,24]} style={{ marginBottom: 16 }}>
        ...
        <Col xs={24} md={8}>
          <Button
            type="primary"
            style={{ width: "100%" }}
            onClick={() => history.push("/supplier/add")}
          >
            Tambah Supplier
          </Button>
        </Col>
